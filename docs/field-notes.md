...
**Incident 43 — after a rebuild the `steakllm` room had no walls for twenty minutes: the NetworkPolicies were the last Application root created** (Sep 5 2026, Step 10.3)
*Symptom:* a server dry-run of `platform/network-policies` said "created" for every policy; `kubectl get networkpolicy -n steakllm` was empty; three Applications (network-policies, gpu-pool, vllm) did not exist; root was `Progressing`, "waiting for healthy state of Application/gpu-mirror".
*Cause:* root syncs its children in waves and does not start a wave until the previous one is Healthy. `network-policies` carried wave 5, with the workloads; the wave-4 `gpu-mirror` Jobs re-ran their guards on the fresh cluster and the image Job took nineteen minutes (a fresh node pulling `crane:debug` and the ECR check through the NAT). Meanwhile every workload of waves 3–4 ran unwalled, and when the policies finally landed they cut Kafka's pods mid-start (the entity operator restarted four times, the operator's reconcile timed out once).
...
**Incident 46 — `cluster-down` refused at its load-balancer check: the cascade had removed the controller before Strimzi's Services, and the Kafka door's two NLBs outlived the workloads** (Sep 6 2026, session end)
*Symptom:* `REFUSING: load balancers still exist; they would outlive the cluster and bill` after the ten-minute wait; two Services in `kafka` with a `deletionTimestamp` and the `service.k8s.aws/resources` finalizer; no controller pod; the `kafka` Application stuck deleting.
*Cause:* the cascade of step 2 treats every workload Application alike. The AWS Load Balancer Controller (10.3) is the only thing that removes an NLB, and it was deleted in the same sweep as Strimzi; Strimzi's LoadBalancer Services then waited forever on the controller's finalizer.
...
- **`TF_VAR_NOTIFY_EMAIL` is not `TF_VAR_notify_email`.** Terraform matches the variable's exact name after `TF_VAR_`; GitHub variables are upper-case by convention, so every one needs a mapping line in the workflow env (as `admin_principal_arn` already had). `No value for required variable` is the tell. (Sep 5)
- **A plan matrix with fail-fast** cancels the siblings when one module fails: "plan (platform) fail" was a cancellation, not a platform problem. Read the failed job first, then the cancelled ones. (Sep 5)
- **Editing YAML with a Python slice** ate two lines under the matrix (`max-parallel`, `concurrency`); `git diff` before commit caught it. One-line edits get `sed` on an exact match or a checked `str.replace`. (Sep 5)
- **checkov CKV2_AWS_5 wants every security group attached within the same plan**; a group that a Kubernetes annotation names by string, or that a later substep attaches, needs a skip with the reason. (Sep 5)
- **BSD `grep -c 'a\|b'`** counts nothing (`\|` is GNU); use `grep -cE 'a|b'`. A "0/12 ok" that was really 12/12. (Sep 5)
...
- **The e2e in CI caught a behaviour change before the cloud did** (Sep 6, 11.3 PR): the gateway started honouring `x-prefer-vllm-seconds`, the summarizer had sent 600 as a hint since Step 6, and every local summary stalled ten minutes. The summarizer now never waits (Bedrock keeps the 90 s promise; vLLM is used whenever it is up); only the eval asks to wait. A CI e2e against the compose stack is the cheapest cloud outage there is.
- **A patch that searches for `)` finds the first one**: the FastAPI instrumentation landed inside the constructor call. Anchor edits on unique multi-line strings, and run the tests before the commit, every time.
- Stumbles: the platform values' `sidecar` and `additionalDataSources` keys already existed (duplicate-key check); tflint's unused-declaration rule caught two leftovers; the mirror Jobs' 500m request could not schedule on the full node and held root's wave gate (gpu-mirror is now wave 6, its requests 100m); the eval's ConfigMap must be regenerated from the script.
