...
| Lambda `steakllm-ingest` | image `steakllm/ingest:lambda-sha-<7>`, arm64, 512 MB, 60 s, in the private subnets; env: bucket, table, `KAFKA_BOOTSTRAP` = the door's NLB (found by tag), `KAFKA_SECURITY_PROTOCOL=SSL`, `KAFKA_CA_SECRET_ID=steakllm/kafka-ca`; two async retries then the DLQ |
