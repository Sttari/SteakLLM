...
> **Status: work in progress — Steps 1–4 of 12 done (repository, AWS bootstrap, CI/CD pipeline, event contracts).** No cluster yet; the only AWS resources are the state bucket, CI roles, budget alarm and five empty ECR repositories. The roadmap below is ticked as steps complete; the plan itself is in [`PLAN.md`](PLAN.md).
